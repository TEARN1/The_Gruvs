...
| C10 | Accessibility coverage ~7% — real inclusion + Play Store listing-quality risk `[confirmed]` | 🟡 | Tab bar already labeled (App.js). RSVP buttons + Touch Down/Crossed Paths (EventDetailScreen.js) 2026-08-18. 6 primary header controls on ProfilePage.js 2026-08-21. **AuthModal.js — FULL coverage, 12/12** 2026-08-21: every user passes through login/signup, was at zero labels; now covers close, tabs (selected state), back, birthday picker, gender/interest pills, email-domain suggestions, show/hide password (icon-only), forgot password, email opt-in (checked state), submit button (dynamic label + busy/disabled state), mode-switch link. ~1,070 `onPress` handlers still unlabeled across the rest of the app (ProfilePage.js's other 92, LandingPage.js, ReelsScreen.js, PostEventModal.js, etc.) — going screen-by-screen, prioritizing full coverage on small critical-path screens over partial coverage on large ones. |
| C11 | `profiles` column-grant lockdowns don't compose — 3 files (`lock_profile_coordinates.sql`, `lock_authenticated_pii.sql`, `regrant_profile_columns.sql`) each independently REVOKE+re-GRANT with a different safe-list; whichever ran last wins `[confirmed]` | 🟢 | Fixed 2026-08-18 — found live: `email`/`push_token`/`phone`/`emergency_contacts`/`siblings` were readable by ANY signed-in user cross-profile (coordinates were correctly locked, PII wasn't). Applied the union of both deny-lists live, verified after. Added `profiles_grants_reconciled.sql` as the one authoritative version (rebuilds full deny-list from `information_schema` every run); marked the composition hazard in all 3 superseded files so it can't recur silently. **Re-run `profiles_grants_reconciled.sql`, not the others, the next time a `profiles` column is added.** |
| C12 | `event_series_followers.series_id` FK references `events(id)`, not `event_series(id)` — a "Follow Tour" row can't actually reference a tour, only a single event `[confirmed]` | 🟢 | Fixed 2026-08-21 — confirmed 0 rows live in `event_series`/`event_series_followers`/`events.series_id` (Tours feature is unused so far), so this was a zero-risk pure schema fix, no data migration needed. Applied live via `apply_migration`; `tour_series.sql` updated to match (both the `CREATE TABLE` and an idempotent `DROP/ADD CONSTRAINT` for anyone already on the old shape). |
