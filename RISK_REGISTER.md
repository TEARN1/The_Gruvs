...
| C12 | `event_series_followers.series_id` FK references `events(id)`, not `event_series(id)` — a "Follow Tour" row can't actually reference a tour, only a single event `[confirmed]` | 🟡 | Found 2026-08-20 while wiring Event Depth Engine Phase 0 CI (the table itself existed live in zero tracked files — added verbatim to `tour_series.sql`, bug included, so CI matches production). Not fixed — verify the client's `followSeries` call actually inserts `series_id = event_series.id` before touching this; if the tours feature is unused live (check row count) the fix is a straight `ALTER ... DROP CONSTRAINT / ADD FOREIGN KEY ... REFERENCES event_series(id)`, same pattern as `fix_series_fk.sql` already used for `events.series_id`. |
