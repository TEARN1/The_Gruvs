...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'business_partnerships') THEN
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS business_partnerships_pair_uniq
...
DROP FUNCTION IF EXISTS find_nearby_events(double precision, double precision, double precision, integer);
...
-- Find nearby vibers
DROP FUNCTION IF EXISTS find_nearby_vibers(uuid, double precision, integer);
...
DROP FUNCTION IF EXISTS find_popular_spots(integer);
...
DROP FUNCTION IF EXISTS find_gruv_hotspots(double precision, double precision, double precision, integer);
