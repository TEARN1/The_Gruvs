...
-- Guarded: only grant functions that already exist, and grant by full
-- signature (every overload) so a fresh build never errors on a not-yet-created
-- or forward-referenced RPC, and a multi-arg name never trips 42725
-- "function name is not unique". (Mirrors the loop in schema_part_4.)
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
