...
CREATE INDEX idx_profiles_vibe       ON public.profiles (vibe_score DESC);
CREATE INDEX idx_profiles_push       ON public.profiles (push_token) WHERE push_token IS NOT NULL;
...
CREATE INDEX idx_events_date      ON public.events (event_date) WHERE deleted_at IS NULL;
CREATE INDEX idx_events_city      ON public.events (lower(city));
CREATE INDEX idx_events_status    ON public.events (status, event_date) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_reels_feed    ON public.reels (created_at DESC) WHERE deleted_at IS NULL;
...
CREATE TABLE public.echoes (                          -- comments on events
  id         UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id   UUID NOT NULL REFERENCES public.events(id) ON DELETE CASCADE,
  user_id    UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  body       TEXT NOT NULL CHECK (length(trim(body)) BETWEEN 1 AND 1000),
...
CREATE INDEX idx_echoes_event ON public.echoes (event_id, created_at DESC);
...
CREATE INDEX idx_messages_thread ON public.messages (sender_id, recipient_id, created_at);
CREATE INDEX idx_messages_inbox  ON public.messages (recipient_id, created_at DESC) WHERE deleted_at IS NULL;
