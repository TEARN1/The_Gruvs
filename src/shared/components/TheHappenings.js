...
import { ACCENT, GOLD, LIGHT_THEME, DARK_THEME } from '../../core/theme';
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: theme.text }]}>The Happenings</Text>
        </View>
        <Text style={[styles.subtitle, { color: theme.textDim }]}>Top 10 real-time</Text>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tierScroll}>
        {TIERS.map(tier => (
          <TouchableOpacity 
            key={tier} 
            style={[styles.tierTab, { backgroundColor: theme.subtle, borderColor: theme.cardBorder }, activeTier === tier && { backgroundColor: GOLD + '22', borderColor: GOLD }]}
            onPress={() => setActiveTier(tier)}
          >
            <Text style={[styles.tierText, { color: theme.textDim }, activeTier === tier && { color: GOLD, fontWeight: '800' }]}>{tier}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
          ...
          <TouchableOpacity key={ev.id} style={styles.eventCard} onPress={() => onCardPress && onCardPress({ id: ev.id, content: { title: ev.title, location: ev.venue } })} activeOpacity={0.85}>
            <ImageBackground source={{ uri: ev.img }} style={styles.cardImg} imageStyle={styles.cardImgInner}>
              <View style={styles.overlay}>
                <View style={[styles.rankBadge, { backgroundColor: theme.bg }]}>
