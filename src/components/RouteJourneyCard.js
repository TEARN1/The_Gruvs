...
export const RouteJourneyCard = ({ route, onPress }) => {
  ...
  const handleJoin = async () => {
    ...
    if (joined) {
      // Leave route
      await supabase.from('route_joins').delete()
        .eq('route_id', route.id).eq('user_id', user.id);
      await supabase.from('routes').update({ join_count: Math.max(0, joinCount - 1) }).eq('id', route.id);
      setJoined(false);
      setJoinCount(c => Math.max(0, c - 1));
    } else {
      // Join route
      await supabase.from('route_joins').upsert(
        { route_id: route.id, user_id: user.id },
        { onConflict: 'route_id,user_id', ignoreDuplicates: true }
      );
      await supabase.from('routes').update({ join_count: joinCount + 1 }).eq('id', route.id);
      setJoined(true);
      setJoinCount(c => c + 1);
